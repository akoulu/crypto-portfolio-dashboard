...
function toObservable(value) {
  const obs = isPromise(value) ? (0, import_rxjs3.from)(value) : value;
...
function composeAsync(validators) {
  ...
  return function(control) {
    ...
    return (0, import_rxjs3.forkJoin)(observables).pipe((0, import_operators.map)(mergeErrors));
  };
}
...
var AbstractControl = class {
  ...
  _events = new import_rxjs3.Subject();
...
var MatListItemBase = class _MatListItemBase {
  ...
  _subscriptions = new import_rxjs4.Subscription();
  ...
  _monitorProjectedLinesAndTitle() {
    this._ngZone.runOutsideAngular(() => {
      this._subscriptions.add((0, import_rxjs4.merge)(this._lines.changes, this._titles.changes).subscribe(() => this._updateItemLines(false)));
    });
  }
